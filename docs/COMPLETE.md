...
| `K_WalkRValue` (DS:0x77fe: 25, 3, 23, 4), `K_WalkUpValue` (0x7808: 2, 9, 2, 9), `K_WalkDownValue` (0x7810: 8, 2, 9, 2) | the player's knight's step per walk frame, indexed by `+0xa` in `KnightWalkRight` (0x4048), `KnightWalkUp` (0x4067) and `KnightWalkDown` (0x4080) | **done**, as `combat::KNIGHT_WALK_R_VALUE`/`KNIGHT_WALK_UP_VALUE`/`KNIGHT_WALK_DOWN_VALUE` and `Fighter::knight_walk_pulse`, moving once a displayed frame by the table entry rather than a flat amount every tick. `ControlBlackKnight` (0x4b79) never calls any of the three, so this applies only to a knight `def.controller=="knight"` and not `flag::DRIVEN`; a computer-controlled knight keeps the flat `speed_x`/`speed_y`, which is what `MoveL`/`MoveR`/the generic up/down movers give it |
