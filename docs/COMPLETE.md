...
| `K_WalkRValue` (DS:0x77fe: 25, 3, 23, 4), `K_WalkUpValue` (0x7808: 2, 9, 2, 9), `K_WalkDownValue` (0x7810: 8, 2, 9, 2) | the player's knight's step per walk frame, indexed by `+0xa` in `KnightWalkRight` (0x4048), `KnightWalkUp` (0x4067) and `KnightWalkDown` (0x4080) | **done**, as `combat::KNIGHT_WALK_R_VALUE`/`KNIGHT_WALK_UP_VALUE`/`KNIGHT_WALK_DOWN_VALUE`, moving once a displayed frame by the table entry. The note here used to add that `ControlBlackKnight` (0x4b79) keeps a flat speed because it calls "the generic movers": **wrong**, and corrected below — those movers are table-driven too, and the black knight has `BKnightWALKR`/`U`/`D`, which hold these same numbers |
| `TroggWALKR` (DS:0x7746), `TroggWALKU` (0x775e), `TroggWALKD` (0x7772), `TrollWALKR` (0x7ba2), `MudmenWALK` (0x7b8e), `BKnightWALKR`/`U`/`D` (0x7bb6, 0x7bca, 0x7bda) | every creature walk-speed table in the image: the step per walk frame that `MoveL`/`MoveR`/`MoveU`/`MoveD` (0x4dd5, 0x4e09, 0x4e3d, 0x4e64) index by the walk cycle, loaded by `TroggMove` (0x2e37, 0x2e43, 0x2e4f, 0x2e5b), `ControlBlackKnight`'s `M0$`..`M3$` (0x4be6, 0x4bf2, 0x4bfe, 0x4c0a), `TrollMoveL`/`TrollMoveR` (0x5648, 0x5667) and `MudmenMoveR`/`MudmenMoveL` (0x5422, 0x5435) | **done**, as `content::WalkSpeed` on each `ActorDef`, read at bake time by `tables::walk_speed` and applied once a displayed frame by `Fighter::walk_pulse`/`walk_step`. Twelve loads in the whole image and no others. Depth where there is no table is the flat literal the controller writes: the troll's ±5 (`ControlTroll` 0x5620, 0x5635), the mudmen's ±2 (`MudmenMoveU`/`MudmenMoveD` 0x5447, 0x5451), the demon's ±5 both ways (`DemonMove` 0x4fe2..0x5001) |
...
| `KnightWyrm` (0xabe8), `MagicCast` slot 0x10 (0xcb60), `StatusDone` (0xbe57), `WyrmFLAG` | the Scroll of the Wyrm | **done**, as `Virtue::Wyrm`, `Cast::Wyrm` and the knight picker's page (`StatTYPE` 0xb, `Screen::AcquirePair`, `App::wyrm_picker`). Casting opens the picker on `InitAKnight`'s own first candidate (0xc90d), `NEXT` cycles it (`dragon::next_wyrm_seat`, the caster's own seat always skipped), and `EXIT` commits whichever seat is highlighted into the dragon's own target (`Flight::wyrm_picked`). `KnightWyrm` (0xabe8) itself is a separate, narrower thing: it only fires on a computer knight's own turn and hands the dragon whichever seat that knight is *already* chasing via `FindKnight`, and its write is provably inert anyway — `ContinueDragon`'s own random reroll, called in the same breath, overwrites it before anything reads it back |
...
- [x] The two that act on the dragon: `TalismanWrym` (0x43f4) shifts the dragon's blow
      ...
      slot 0x10 (0xcb60), spending its charge at once and opening `Virtue::Wyrm`'s knight
      picker (`StatTYPE` 0xb, `App::wyrm_picker`) on `InitAKnight`'s own first candidate
      (0xc90d/0xc913, `dragon::next_wyrm_seat`, the caster's own seat always skipped);
      `EXIT` closing the page is `StatusDone` (0xbe57), `Flight::wyrm_picked`, writing
      whichever seat was last highlighted into the dragon's `+0x46` — nothing here spends
      a second charge or sets it flying, both already having happened elsewhere. See 3.3
